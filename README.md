...
> **注意**: uzura は AI エージェント用途に特化しているため、JS からのネットワークリクエストに CORS（同一オリジンポリシー）を適用しません。一方で SSRF 対策として、JS（fetch / XMLHttpRequest）からのリクエストはループバック・リンクローカル（クラウドメタデータ含む）・RFC1918 プライベートアドレスなど内部向け宛先へは既定でブロックされます（`Options.AllowPrivateNetworkJS` でオプトアウト可能）。トップレベルの `Navigate` はこの制限を受けません。
