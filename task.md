...
- [ ] `go mod init github.com/nyasuto/uzura`
- [ ] ディレクトリ作成: `cmd/uzura/main.go`, `internal/dom/`, `internal/html/`
- [ ] `golang.org/x/net/html` を依存に追加
- [ ] `parse` サブコマンド付きの基本CLI（stdinからHTMLを読みDOM出力）
- [ ] `go build ./cmd/uzura` でバイナリ生成確認
- [ ] `uzura version` でバージョン出力
