...
- [x] SVG データURI の除去（Svelte等で混入）
- [x] 過剰な空行・空白の正規化
- [x] 出力トークン数の上限設定（`--max-tokens` オプション）
- [x] テスト: 各サイトカテゴリでの品質スコア改善確認
...
- [x] レスポンスの最大サイズ設定（デフォルト: 100KB）
- [x] 超過時の切り詰め（末尾に `[truncated]` 付与）
- [x] `browse` ツールに `max_length` パラメータ追加
- [x] テスト: 巨大ページでの切り詰め動作確認

### Task 15.2: query ツールの大量結果対策

- [x] `query` ツールに `limit` パラメータ追加（デフォルト: 100件）
- [x] `offset` パラメータでページネーション対応
- [x] 結果件数のサマリ情報を付与（`total: 1178, returned: 100`）
- [x] テスト: 1000件超のリンクがあるページでの動作

### Task 15.3: タイムアウトとリトライ

- [x] MCP ツール実行のタイムアウト設定（デフォルト: 30秒）
- [x] ネットワークリトライ（最大2回、指数バックオフ）
- [x] タイムアウト時の部分結果返却（取得済みのDOMを返す）
- [x] テスト: 遅延サーバー、タイムアウトシナリオ
...
- [x] `semantic_tree` の基本テスト（ランドマーク・インタラクティブ要素の検出）
- [x] `query` の基本テスト（h1取得、リンク数取得、属性取得）
- [x] `query` の limit/offset ページネーションテスト（Phase 15 の検証）
- [x] 巨大DOM（1000要素超）での安定性テスト
